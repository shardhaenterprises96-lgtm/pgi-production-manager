...
- **Backup & Restore (admin-only)**: Three tabs — (1) Manual backup download (.zip/.json) of all company-scoped data plus automatic-backup cadence preferences (daily/weekly/monthly — persisted only; scheduled execution deferred) and backup history; (2) Restore — upload a .zip/.json backup to replace all current company data (SERIALIZABLE delete-then-insert, strict same-company guard, AlertDialog confirm); (3) Reset — wipe all business data to start fresh while preserving logins/permissions/settings (typed "RESET" + AlertDialog confirm). Both restore and reset are strictly company-scoped via the COMPANY_TABLES allow-list; restore builds insert columns only from information_schema metadata (never from uploaded keys) to prevent SQL injection.
